...
export default function DonorDiscovery({
  ...
  /* ── IRS outreach email send (EmailJS) ── */
  const [irsRecipientEmail, setIrsRecipientEmail] = useState('');
  const [irsSendStatus, setIrsSendStatus] = useState<'idle' | 'sending' | 'sent' | 'error'>('idle');
  const [irsSendError, setIrsSendError] = useState('');
  const [irsSentTotal, setIrsSentTotal] = useState<number>(() => {
  ...
  const fetchIRS = async (q: string, state: string, ntee: string, type: string, page: number) => {
    ...
    // Helper: hit ProPublica with a single query string, optionally with state/ntee filters.
    // Brackets in state[id] / ntee[id] must be URL-encoded per ProPublica docs.
    const fetchPage = async (queryStr: string, pg: number) => {
      let url = `/api/propublica/search.json?q=${encodeURIComponent(queryStr)}&page=${pg - 1}`;
    ...
    try {
      // 1. Try the user's full query first (preferred — exact phrase match).
      let data = await fetchPage(effectiveQuery, page);
      let usedFallback = false;

      // 2. Fallback: ProPublica treats multi-word queries as AND across the org's name,
      //    so combining a state filter with multi-word queries (e.g. "children healthcare" + TX)
      //    often returns 0 hits even though each word individually matches hundreds of orgs.
      //    When that happens, fan-out a request per token and merge unique results,
      //    ranking by how many of the user's terms appear in each org's name.
      const tokens = effectiveQuery.split(/\s+/).filter((t) => t.length >= 3);
      if ((data.total_results ?? 0) === 0 && state && tokens.length > 1) {
        const perToken = await Promise.all(
          tokens.map((t) =>
            fetchPage(t, 1).catch(() => ({ organizations: [], total_results: 0, num_pages: 0 }))
          )
        );

        const merged = new Map<string, { org: any; matches: number }>();
        for (const r of perToken) {
          for (const o of r.organizations || []) {
            const key = String(o.ein);
            if (!merged.has(key)) merged.set(key, { org: o, matches: 0 });
          }
        }
        // Score by how many query tokens appear in the org name (case-insensitive)
        for (const m of merged.values()) {
          const lowerName = (m.org.name || '').toLowerCase();
          m.matches = tokens.filter((t) => lowerName.includes(t.toLowerCase())).length;
        }
        const mergedOrgs = [...merged.values()]
          .sort(
            (a, b) =>
              b.matches - a.matches ||
              ((b.org.score || 0) - (a.org.score || 0))
          )
          .map((m) => m.org);
        ...
        if (mergedOrgs.length > 0) {
          data = {
            ...
            total_results: mergedOrgs.length,
            num_pages: 1,
          };
          usedFallback = true;
        }
      ...
      const numPages = usedFallback ? 1 : (data.num_pages ?? Math.ceil(total / 25));
      setIrsTotalResults(total);
      setIrsTotalPages(numPages);
      setIrsPage(usedFallback ? 1 : page);
      setIrsResults(orgs);
  ...
  /* ── Auto re-fetch when State / Category / Type filter changes ─────────────
     Only kicks in after the user has performed at least one explicit search,
     so the live IRS results stay in sync with the dropdowns state-by-state. */
  useEffect(() => {
    if (!irsSearched) return;
    const id = setTimeout(() => {
      fetchIRS(irsQuery, irsStateFilter, irsNTEEFilter, irsTypeFilter, 1);
    }, 250);
    return () => clearTimeout(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [irsStateFilter, irsNTEEFilter, irsTypeFilter]);
  ...
  /**
   * Sends the currently generated IRS outreach email via EmailJS.
   * IRS 990 data does not include public contact emails for nonprofits,
   * so the user supplies the recipient address manually.
   */
  const sendIRSOutreachEmail = async () => {
    if (!selectedOrgForEmail || !irsEmailContent.trim()) return;
    setIrsSendError('');
    setIrsSendStatus('sending');

    // Pull "Subject: …" from the first non-empty line if present, otherwise fall back.
    const lines = irsEmailContent.split('\n');
    let subject = `Partnership Inquiry — PediPlace x ${selectedOrgForEmail.name}`;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lines[i].trim()) break; // first content line wasn't a subject — keep default
    }
    const body = lines.slice(bodyStartIndex).join('\n').replace(/^\s*\n+/, '');

    const result = await sendOutreachEmail({
      toEmail:      irsRecipientEmail,
      toName:       selectedOrgForEmail.name,
      organization: selectedOrgForEmail.name,
      subject,
      body,
    });

    if (result.ok) {
      setIrsSendStatus('sent');
      setIrsSentTotal((n) => {
        const next = n + 1;
        try { localStorage.setItem('pediplace_outreach_sent_total', String(next)); } catch { /* ignore */ }
        return next;
      });
      // If the org is in the pipeline, bump emailAttempts and mark contacted.
      const inPipeline = irsPipeline.find((p) => p.org.ein === selectedOrgForEmail.ein);
      if (inPipeline) {
        updateIRSPipelineEntry(inPipeline.id, {
          emailAttempts: inPipeline.emailAttempts + 1,
          status: inPipeline.status === 'cold' ? 'contacted' : inPipeline.status,
        });
      }
    } else {
      setIrsSendStatus('error');
      setIrsSendError(result.error || 'Failed to send email.');
    }
  };
              ...
              <>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl p-5">
                      ...
                      {/* Active filter chips — make state filtering obvious */}
                      {(irsStateFilter || irsNTEEFilter || irsTypeFilter !== 'All Types') && (
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-600">Active filters:</span>
                          {irsStateFilter && (
                            <button
                              onClick={() => setIrsStateFilter('')}
                              title="Clear state filter"
                              className="inline-flex items-center gap-1 text-[11px] font-semibold bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/20 px-2 py-0.5 rounded-full hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors"
                            >
                              <MapPin className="w-3 h-3" />
                              {US_STATES.find(([code]) => code === irsStateFilter)?.[1] || irsStateFilter}
                              <X className="w-3 h-3 opacity-60" />
                            </button>
                          )}
                          {irsNTEEFilter && (
                            <button
                              onClick={() => setIrsNTEEFilter('')}
                              className="inline-flex items-center gap-1 text-[11px] font-semibold bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/20 px-2 py-0.5 rounded-full hover:bg-purple-100 dark:hover:bg-purple-500/20 transition-colors"
                            >
                              {NTEE_CATEGORIES.find((c) => c.id === irsNTEEFilter)?.label || irsNTEEFilter}
                              <X className="w-3 h-3 opacity-60" />
                            </button>
                          )}
                          {irsTypeFilter !== 'All Types' && (
                            <button
                              onClick={() => setIrsTypeFilter('All Types')}
                              className="inline-flex items-center gap-1 text-[11px] font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20 px-2 py-0.5 rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors"
                            >
                              {irsTypeFilter}
                              <X className="w-3 h-3 opacity-60" />
                            </button>
                      ...
                      {irsLoading && (
                        <div className="flex flex-col items-center gap-3 mt-4 py-10">
                          ...
                          <span className="text-sm font-semibold text-gray-600 dark:text-slate-400">
                            {irsStateFilter
                              ? `Querying IRS 990 database for ${US_STATES.find(([code]) => code === irsStateFilter)?.[1] || irsStateFilter} nonprofits…`
                              : 'Querying IRS 990 database via ProPublica…'}
                          </span>
                        ...
                        <>
                          ...
                          {irsTotalResults > 0 && (
                            <p className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-4">
                              ...
                              {irsStateFilter && ` in ${US_STATES.find(([code]) => code === irsStateFilter)?.[1] || irsStateFilter}`}
                      ...
                      {/* Empty state — shown when the user has searched but no results came back */}
                      {!irsLoading && irsSearched && irsResults.length === 0 && (
                        <div className="flex flex-col items-center gap-2 py-10 text-center">
                          <div className="w-12 h-12 rounded-2xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center mb-1">
                            <Search className="w-6 h-6 text-gray-300 dark:text-slate-600" />
                          </div>
                          <p className="text-sm font-semibold text-gray-500 dark:text-slate-400">
                            No nonprofits found
                            {irsStateFilter && ` in ${US_STATES.find(([code]) => code === irsStateFilter)?.[1] || irsStateFilter}`}
                            {irsQuery.trim() && ` for "${irsQuery.trim()}"`}
                          </p>
                          <p className="text-xs text-gray-400 dark:text-slate-600 max-w-md">
                            Try a different state, broaden the keywords, or clear filters above.
                          </p>
                          {(irsStateFilter || irsNTEEFilter || irsTypeFilter !== 'All Types') && (
                            <button
                              onClick={() => {
                    ...
                    <div ref={irsEmailStepRef} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 scroll-mt-4">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        {/* Tiny gear — opens EmailJS setup. No-op visual when configured. */}
                        <button
                          onClick={() => setShowEmailJSSetup((v) => !v)}
                          title={emailjsReady ? 'EmailJS settings' : 'Set up EmailJS'}
                          className={`${selectedOrgForEmail ? '' : 'ml-auto'} w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 dark:text-slate-600 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors`}
                        >
                          <span className="text-base leading-none">⚙</span>
                        </button>
                      </div>

                      {/* ── EmailJS setup panel (hidden by default; opens via gear icon) ── */}
                      {!emailjsReady && !showEmailJSSetup && (
                        <div className="flex items-center gap-2 mb-4 text-[11px] text-gray-400 dark:text-slate-600">
                          <button
                            onClick={() => setShowEmailJSSetup(true)}
                            className="font-semibold text-gray-500 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 underline-offset-2 hover:underline transition-colors"
                          >
                            ⚙ Set up EmailJS
                          </button>
                          <span>— optional, only needed if you want to send emails directly from this app.</span>
                        </div>
                      )}
                      ...
                      {showEmailJSSetup && (
                        <div className="mb-4 bg-amber-50/60 dark:bg-amber-500/5 border-2 border-amber-300 dark:border-amber-500/30 rounded-xl p-4">
                          <div className="flex items-start gap-2 mb-3">
                            ...
                            <button
                              onClick={() => { setShowEmailJSSetup(false); setEjSaveStatus('idle'); }}
                          ...
                          <div className="flex items-center gap-2 mt-3 flex-wrap">
                            ...
                            {emailjsReady && (
                              <button
                                ...
                                className="text-[11px] font-semibold text-gray-500 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                              >
                                Reset to defaults
                              </button>
                            )}
                      ...
                      {!irsEmailLoading && irsEmailContent && (
                        <>
                          <textarea value={irsEmailContent} onChange={(e) => { setIrsEmailContent(e.target.value); setIrsSendStatus('idle'); }}
                            rows={14}
                          ...
                          {/* ── Send via EmailJS ── */}
                          <div className="bg-blue-50/40 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20 rounded-xl p-3 mb-3">
                            <div className="flex items-center gap-2 mb-2">
                              <Send className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                              <span className="text-[11px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-300">Send via EmailJS</span>
                              <span className="text-[10px] text-gray-400 dark:text-slate-600 ml-1">— IRS 990 doesn't expose contact emails, so paste the recipient below</span>
                              <span className="ml-auto text-[10px] font-semibold text-gray-400 dark:text-slate-600">Total sent: <strong className="text-blue-600 dark:text-blue-400">{irsSentTotal}</strong></span>
                            </div>
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                              <div className="relative flex-1">
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-slate-600" />
                                <input
                                  type="email"
                                  value={irsRecipientEmail}
                                  onChange={(e) => { setIrsRecipientEmail(e.target.value); setIrsSendStatus('idle'); setIrsSendError(''); }}
                                  placeholder="grants@example.org"
                                  className="w-full text-sm bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-slate-200 pl-9 pr-3 py-2 rounded-lg focus:outline-none focus:border-blue-400"
                                  disabled={irsSendStatus === 'sending'}
                                />
                              </div>
                              <button
                                onClick={sendIRSOutreachEmail}
                                disabled={!emailjsReady || irsSendStatus === 'sending' || !irsRecipientEmail.trim()}
                                className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors whitespace-nowrap"
                              >
                                {irsSendStatus === 'sending'
                            ...
                            {irsSendStatus === 'sent' && (
                              <p className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 mt-2">
                                ✓ Email sent to {irsRecipientEmail}
                              </p>
                            )}
                            {irsSendStatus === 'error' && (
                              <p className="text-[11px] font-semibold text-red-600 dark:text-red-400 mt-2">
                                Failed to send: {irsSendError}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <button onClick={() => { setSelectedOrgForEmail(null); setIrsSendStatus('idle'); setIrsRecipientEmail(''); }} className="ml-auto text-xs text-gray-400 dark:text-slate-600 hover:text-gray-600 transition-colors">Clear</button>
                          </div>
                        </>
                      )}
                    </div>
